...
fn get_internal_data(limit: i32) -> Vec<Message> {
  ...
  let mut stmt = conn.prepare("SELECT * FROM message ORDER BY time_stamp DESC LIMIT :limit").expect("SELECT * FROM message statement failed.");
  for msg in stmt.query_map(&[(":limit", limit.to_string().as_str())], |row|{
    Ok(
      Message {
        id: row.get(0)?,
        author: row.get(1)?,
